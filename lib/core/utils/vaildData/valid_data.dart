...
import 'package:mostaql/core/utils/safe_utils.dart';

bool validString(dynamic src) => src?.toString().isNotEmpty == true;

String validateString(dynamic src, [String def = '']) => validString(src) ? src : def;
...
T? validateJson<T>(dynamic src, T Function(Map<String, dynamic> json) fromJson) =>
...
List<T> validateJsonList<T>(dynamic src, T Function(Map<String, dynamic> e) fromJson) =>
    validList<Map<String, dynamic>>(src) ? (src as List<Map<String, dynamic>>).map(fromJson).cast<T>().toList() : <T>[];

bool validEmail(String src) =>
    RegExp(r"""^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[a-zA-Z0-9]+\.[a-zA-Z]+""").hasMatch(src);
